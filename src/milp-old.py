...
class xgboost_wrapper:
    def __init__(self, model, binary=False, max_trees=None):
        self.model = model
        self.binary = binary
    ...
    def predict(self, input_data, pred_leaf=False):
        ...
        if max_trees is not None and max_trees != -1: #NV
            boosted_rounds = self.model[0].num_boosted_rounds()
            max_trees = min(max_trees, boosted_rounds)
        if max_trees is None or max_trees == -1:
            test_predict = np.array(self.model[0].predict(input_data, pred_leaf=pred_leaf))
        else:
            test_predict = np.array(
                self.model[0].predict(input_data, iteration_range=(0, max_trees), pred_leaf=pred_leaf)
            )
        return test_predict
...
class xgbKantchelianAttack(object):
    ...
    def __init__(
        ...
        self.debug = debug
        self.args = args
        self.n_classes = model.model[-4]
        self.multiclass = self.args["multiclass"]
        self.strongmulti = self.args["strong_multi"]
        self.guard_val = guard_val
        self.round_digits = round_digits
        self.model = model
        self.base_val = sigmoid_inv(model.model[-3])
        ...
        if self.binary:
            ...
            model.model[0].dump_model(temp, dump_format="json")
            with open(temp) as f:
        ...
        if self.args["in_distro_clauses"]:
            featureDict = self.model.model[6]
    ...
    #     n_features = self.model.model[3]
    #     trees = self.model.model[1]
    #     sensitive_features = self.varyingFeat
    #     feature_list = [""]*n_features
    #     featurenames = self.model.model[6]
    ...
    def attack(self,options):
        ...
        if not args["precise"]:
                    ...
                    if self.args["otherlabel"] == -1:
                        ...
                        for i in range(self.n_classes):
                            ...
                            self.m.addConstr(
                                ...
                                >= self.guard_val * self.args["precision"],
                                name=f"mislabel_{i}",
                            )
                        for i in range(self.n_classes):
            ...
            if self.multiclass:
                ...
                self.m.addConstr(
                    LinExpr(up_weights, self.llist) + self.base_val >= self.guard_val, name="mislabel"
                )
                self.m.addConstr(
                    LinExpr(down_weights, self.llist2) +self.base_val <= -self.guard_val,
                    name="mislabel-2",
                )
        ...
        x =[0] * self.model.model[0].num_features()
        x2 =[0] * self.model.model[0].num_features()
        for i in range(1):
        ...
        xbound = {}
        x2bound = {}
        features = list(self.model.model[6].keys())
        op_range_list = self.model.model[7]
        # print(op_range_list)
        ...
        op_range_list = []
        for j in range(self.model.model[0].num_features()):
        ...
        if args["compute_data_distance"]:
            # data_distance.compute_data_distance(x,self.varyingFeat,self.model.model[6],self.model.model[3],self.model.model[1],options)
            # print(f"=========================================")
            print(f"First region = {xbound}")
            # print(f"Second region = {x2bound}")
            data_distance.compute_data_distance(xbound,self.varyingFeat,self.model.model[6],self.model.model[3],self.model.model[1],options,dist_type='L0')
            data_distance.compute_data_distance(xbound,self.varyingFeat,self.model.model[6],self.model.model[3],self.model.model[1],options,dist_type='L1')
            data_distance.compute_data_distance(xbound,self.varyingFeat,self.model.model[6],self.model.model[3],self.model.model[1],options,dist_type='L2')
            data_distance.compute_data_distance(xbound,self.varyingFeat,self.model.model[6],self.model.model[3],self.model.model[1],options,dist_type='Linf')
        ...
        # utils.dump_dotty(self.model.model[0])
...
def main(args_inp,options):
    ...
    if args["filenum"].isdigit():
        bst = open_model(model_files[int(args["filenum"])], args["max_trees"])
    else:
        bst = open_model(args["filenum"], args["max_trees"],details_file = args['details']) #NV
    model = xgboost_wrapper(bst, binary=True, max_trees=args["max_trees"])
    if not args["all_features"]:
        varyingFeat = args["features"]
    else:
        varyingFeat = [i for i in range(0, model.model[3])]
    if args["precision"] < 0:
