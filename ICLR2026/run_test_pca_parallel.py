...
def main():
    ...
    if all_tasks:
        with multiprocessing.Pool(processes=effective_workers) as pool:
            for row in pool.imap_unordered(_worker_run_variant, all_tasks):
                ...
                # Live console output
                print(f"[Done] {row['dataset_key']} | feat={row['feature']} | gap=({row['lgap']},{row['ugap']}) | var={row['variant']}")
                
                if args.show_output:
                    if row.get("stdout"): print(row["stdout"])
                    if row.get("stderr"): print(row["stderr"])
                ...
                if row["status"] == "missing_clause_file":
                    ...
                    print_variant_metrics(row["dataset_key"], case_mock, row["variant"], _metrics_from_row(row))
                    print()
