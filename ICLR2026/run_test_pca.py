...
def print_variant_metrics(dataset_key, case, variant_name, metrics):
    print(
        f"{dataset_key} | feature={case['feature']} | gap=({case['lgap']}, {case['ugap']}) "
        f"| variant={variant_name}"
    )
    print(f"  Pairwise L2 distance: {metrics['pair_l2']}")
    print(f"  Point1 L1/L2 to centroid: {metrics['point1']['l1']} / {metrics['point1']['l2']}")
    print(f"  Point2 L1/L2 to centroid: {metrics['point2']['l1']} / {metrics['point2']['l2']}")
    print(
...
def main():
    ...
    for modelname, modeltype in benchmarks:
        ...
        for case in cases:
            ...
            for variant in case["variants"]:
                print(f"  Variant {variant['name']}: {variant['command']}")

                clause_path = ROOT_DIR / clause_file
                if variant["use_clause"] and not clause_path.exists():
                    row = {
                        "dataset": modelname,
                        "modeltype": modeltype,
                        "dataset_key": dataset_key,
                        ...
                        "command": variant["command"],
                        "rc": None,
                        "status": "missing_clause_file",
                        "error": f"Missing clause file: {clause_path}",
                    }
                ...
                res = run_command(variant["command_parts"], show_output=args.show_output)
                row = variant_row(dataset_key, modelname, modeltype, case, variant, clause_file, res)
                ...
                try:
                    point1, point2, point_source = extract_solution_points(
                        res.stdout,
                        prefer_pca_points=variant["use_pca"],
                        dataset_to_model_indices=dataset_to_model_indices,
                    )
                    if len(point1) != len(point2) or len(point1) != len(centroid):
                        raise ValueError("Dimension mismatch in extracted points")
                ...
                print_variant_metrics(dataset_key, case, variant["name"], metrics)
            print()

        if not args.no_run:
            dataset_summary(dataset_key, dataset_rows)
            print()
